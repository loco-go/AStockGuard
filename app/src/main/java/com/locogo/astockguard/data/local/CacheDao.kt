...
interface CacheDao {
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun upsertSignalState(item: SignalStateEntity)
    @Query("SELECT * FROM signal_state WHERE code = :code LIMIT 1")
    suspend fun getSignalState(code: String): SignalStateEntity?
    @Query("SELECT * FROM signal_state ORDER BY lastTransitionAt DESC")
    suspend fun getSignalStates(): List<SignalStateEntity>
